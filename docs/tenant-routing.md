...
resolve a destination. Hosted MCP model/tool input addresses only a logical
target inside the authenticated session:

```json
{ "destination": { "type": "child", "key": "alex" } }
```

The family identity comes from the bearer session, not from model/tool input.
The trusted Discord adapter API may carry that authenticated family context
internally, but it must never be supplied as an untrusted model selector. No
caller may choose an arbitrary raw Discord channel id. Provider ids are stored
only in an operator-managed binding and are resolved by the trusted family
route index (`packages/core`). A binding has:
...
Every request path must carry the authenticated `familyId` in server-side
context, then resolve the logical destination inside that family. Unknown
destinations, duplicate family or child ids, and duplicate destination keys are
hard errors. Never fall back to a global provider-id lookup.
