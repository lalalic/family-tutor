...
3. Discord redirects back to the personalized `/setup/<claim>` setup hub. The public `/setup` page and the personalized page share one canonical setup checklist. The personalized page shows the configured kids, extension download, and two ways to continue: **Manual setup** or **Setup with Codex**.
...
5. Setup with Codex gives Codex the current setup URL and instructs it to open that server page as the authoritative manual. Codex follows the same checklist as a person, uses extension Setup Help for browser/ChatGPT-specific actions, refreshes live setup state before Project setup and finish, and stops only for Discord/ChatGPT security or authorization confirmations. It must not print or persist setup claims, auth/refresh tokens, provider IDs, or child conversation content.
...
7. Claiming establishes the family session even if Discord parent/kid channels are still being prepared. The extension shows a waiting state, polls setup readiness, and does not start ChatGPT Project setup until the channels are ready. Each status refresh also refreshes the current learner list, so kids added after claim become required Project-linking work before setup can finish.
