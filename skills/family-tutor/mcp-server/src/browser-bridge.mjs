...
export class BrowserBridge {
  ...
  channelHandle(providerChannelId){
    ...
    const handle=`ch_${crypto.createHmac('sha256',this.token).update(`channel:${providerId}`).digest('base64url').slice(0,24)}`;
  ...
  async send(channelId,text){
    const providerChannelId=this.channelHandles.get(String(channelId||'').trim());
