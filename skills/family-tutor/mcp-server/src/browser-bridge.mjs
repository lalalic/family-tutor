...
function contextWithCorrelation(prompt,correlationId){
  ...
  return `<FAMILY_TUTOR_CONTEXT>\n${JSON.stringify({type:envelope.type,data:{correlationId,...envelope.data}})}\n</FAMILY_TUTOR_CONTEXT>`;
}
...
export class BrowserBridge {
  ...
  #signRefreshToken({clientId,scope,resource,familyId=null,expiresIn=180*24*60*60}){
    const payload=b64url(JSON.stringify({iss:this.publicOrigin,client_id:clientId,scope,resource,...(familyId?{family_id:familyId}:{}),exp:Math.floor(Date.now()/1000)+expiresIn}));
    const signature=crypto.createHmac('sha256',this.token).update(`ftr1.${payload}`).digest('base64url');
    return `ftr1.${payload}.${signature}`;
  }
  #verifyRefreshToken(token,{clientId,scope,resource,familyId=null,allowMissingResource=false}){
    const parts=String(token||'').split('.');
    ...
    try{
      const payload=JSON.parse(Buffer.from(parts[1],'base64url').toString('utf8'));
      const resourceOk=payload.resource===resource||(allowMissingResource&&payload.resource==null);
      return payload.iss===this.publicOrigin&&payload.client_id===clientId&&payload.scope===scope&&resourceOk&&(!familyId||payload.family_id===familyId)&&Number(payload.exp)>Math.floor(Date.now()/1000);
    }catch{return false;}
  }
  #signExtensionRefreshToken(familyId,expiresIn=180*24*60*60){return this.#signRefreshToken({clientId:this.extensionOAuthClientId,scope:'extension_refresh',resource:this.#extensionResource(),familyId,expiresIn});}
  ...
  async #oauthToken(req,res){
    ...
    if(grantType==='refresh_token'){
      ...
      if(credentials.id===this.extensionOAuthClientId){
        if(!this.#verifyExtensionRefreshToken(refreshToken)) return json(res,400,{error:'invalid_grant'});
        return json(res,200,{access_token:this.#signAccessToken({scope:'extension',resource:this.#extensionResource(),clientId:this.extensionOAuthClientId,familyId:this.#familyId()}),refresh_token:this.#signExtensionRefreshToken(this.#familyId()),token_type:'Bearer',expires_in:3600,scope:'extension'});
      }
      if(credentials.id===this.oauthClientId){
        if(!this.#verifyChatGptRefreshToken(refreshToken)) return json(res,400,{error:'invalid_grant'});
        return json(res,200,{access_token:this.#signAccessToken({scope:'tutor',resource:this.#oauthResource(),clientId:this.oauthClientId}),refresh_token:refreshToken,token_type:'Bearer',expires_in:3600,scope:'tutor'});
      }
    ...
    else if(record.clientId===this.oauthClientId) response.refresh_token=this.#signChatGptRefreshToken();
  ...
  async #mcp(body){
    ...
    if(method==='tools/list') return {jsonrpc:'2.0',id,result:{tools:[
      {name:'reply_to_discord',title:'Reply to Discord',description:'Reply to the Discord destination associated with the active Family Tutor correlation id. Use final=false for progress and final=true for the final response.',securitySchemes:[{type:'oauth2',scopes:['tutor']}],annotations:{readOnlyHint:false,destructiveHint:false,openWorldHint:false,idempotentHint:false},_meta:{securitySchemes:[{type:'oauth2',scopes:['tutor']}],ui:{visibility:['model','app']},'openai/toolInvocation/invoking':'Sending Family Tutor reply…','openai/toolInvocation/invoked':'Family Tutor reply sent'},inputSchema:{type:'object',additionalProperties:false,required:['correlationId','text'],properties:{correlationId:{type:'string',minLength:1,maxLength:160,description:'Opaque reply handle supplied in data.correlationId for the active inbound turn.'},text:{type:'string',minLength:1,maxLength:12000,description:'Message text to send.'},final:{type:'boolean',default:true,description:'Set false for progress and true to complete the active turn.'}}}},
