...
test('Discord install creates one-time family claim and family-scoped extension session',async()=>{
  ...
  try{
    ...
    const deniedInstall=await fetch(`${bridge.endpoint()}/discord/install`,{redirect:'manual'});
    ...
    assert.match(setupHtml,/\/downloads\/family-tutor-extension-2\.6\.9\.zip/);
    ...
    assert.match(setupHtml,/Exceptions and recovery/);
    assert.match(setupHtml,/A kid is added after extension claim/);
